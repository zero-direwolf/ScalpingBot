...
import urllib
import math
from util.api_key import generate_nonce, generate_signature #何で必要か不明
...
class BitMEXWebsocket():
    ...
    def loop(self):
        ...
        while True:
            current_value = self.recent_trade(self.symbol)['price']
            pos = self.position(self.symbol)['currentQty']
            print("current_value:" + str(current_value),"pos:" + str(pos))
            time.sleep(1)
            
        #self.get_instrument(self, self.symbol)
    #
    # method
    #
    ...
    def recent_trade(self, symbol):
        '''Get recent trades.'''
        recent_trades = self.data['trade']
        current_value = [p for p in recent_trades if p['symbol'] == symbol]
        if len(current_value) == 0:
            # No position found; stub it
            return {'timestamp': 0, 'symbol': symbol, 'side': "", 'size': 0, 'price': 0, 'tickDirection': "", 'trdMatchID': "", 'grossValue': 0, 'homeNotional': 0, 'foreignNotional': 0}
